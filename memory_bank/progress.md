...
| AI-ассистент (OpenRouter API) | ✅ Интегрирован | 100% |
...
### 2026-04-02 — AI: OpenRouter
- Сервис `openrouter_ai_service.dart` (REST `chat/completions`), ключ в `lib/core/config/openrouter_user_key.dart`, опционально SharedPreferences и `OPENROUTER_API_KEY` / `OPENROUTER_MODEL` / `OPENROUTER_HTTP_REFERER`
- Удалены интеграции DeepSeek и Gemini из активного кода чата

### 2026-03-20 — AI помощник (исторически DeepSeek)
- Конфигурация через dart-define и ключ в приложении
- Контекст расширен данными пользователя из `UserAccount` (подписка, рейтинг, средний процент техники)
...
### AI-ассистент (OpenRouter API)
- Интеграция OpenRouter (OpenAI-совместимый endpoint)
