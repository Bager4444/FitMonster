...
void main() {
  group('Accessibility Tests', () {
    testWidgets('Проверка семантических меток для кнопок', (tester) async {
      ...
      expect(find.byTooltip('Добавить продукт'), findsOneWidget);
      expect(find.byTooltip('Открыть камеру'), findsOneWidget);
    });
    ...
    testWidgets('Проверка размеров touch targets', (tester) async {
      ...
      final smallButton = find.byKey(Key('small_button'));
    ...
    testWidgets('Проверка поддержки screen reader', (tester) async {
      ...
      expect(find.bySemanticsLabel(RegExp(r'Текущий вес: 70 килограмм')), findsOneWidget);
      expect(find.bySemanticsLabel(RegExp(r'Кнопка добавить упражнение')), findsOneWidget);
      expect(find.bySemanticsLabel(RegExp(r'Прогресс выполнения: 75 процентов')), findsOneWidget);
    });
