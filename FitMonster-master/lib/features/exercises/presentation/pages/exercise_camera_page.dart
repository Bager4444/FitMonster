...
class _ExerciseCameraPageState extends State<ExerciseCameraPage> {
  ...
  // FPS мониторинг и оптимизация
  int _frameCount = 0;
  DateTime _lastFpsUpdate = DateTime.now();
  double _currentFps = 0.0;
  
  // Ультра-оптимизированные настройки для 7+ FPS
  static const int _targetFps = 8; // Повышаем целевой FPS до 8
  static const int _frameSkipRate = 2; // Обрабатываем каждый 2-й кадр вместо 3-го
  static const int _mlProcessInterval = 2; // ML Kit каждый 2-й обработанный кадр
  
  // Улучшенный счетчик повторений
  final ImprovedRepCounter _repCounter = ImprovedRepCounter();
  
  // Флаг обработки кадра с таймаутом
  bool _isProcessingFrame = false;
  DateTime _lastFrameTime = DateTime.now();
  int _frameSkipCounter = 0;
  int _mlProcessCounter = 0;
  
  // Кэш для оптимизации конвертации изображений
  final Map<String, Uint8List> _imageBufferCache = {};
  DateTime _lastCacheClean = DateTime.now();
  
  // Система опыта
  int _lastRepCountForExp = 0;
  int _lastTimeForExp = 0;
  ...
  void _initializePoseDetector() {
    try {
      ...
      // Используем базовую модель для максимальной скорости
      final options = PoseDetectorOptions(
        mode: PoseDetectionMode.stream,
        model: PoseDetectionModel.base, // Базовая модель быстрее accurate
      );
      _poseDetector = PoseDetector(options: options);
      print('✅ ML Kit PoseDetector initialized (ULTRA-FAST: stream mode, BASE model)');
    } catch (e) {
  ...
  Future<void> _initializeCamera() async {
    try {
      ...
      _cameraController = CameraController(
        frontCamera,
        ResolutionPreset.low, // Используем низкое разрешение для максимального FPS
        enableAudio: false,
  ...
  void _startImageStream() {
    ...
    print('📷 Starting ULTRA-OPTIMIZED camera stream (8 FPS target, smart processing)');
    
    _cameraController!.startImageStream((CameraImage image) {
      // Быстрая проверка - пропускаем если предыдущий кадр еще обрабатывается
      ...
      // Проверка FPS с более агрессивным таймингом
      final now = DateTime.now();
      final timeSinceLastFrame = now.difference(_lastFrameTime).inMilliseconds;
      final minFrameInterval = 1000 ~/ _targetFps; // ~125ms для 8 FPS
      ...
      // Асинхронная обработка с таймаутом
      _processImageUltraFast(image).timeout(
        const Duration(milliseconds: 100), // Таймаут 100ms
        onTimeout: () {
        ...
        _isProcessingFrame = false;
        print('❌ Error in ultra-fast processing: $e');
      });
  ...
  Future<void> _processImageUltraFast(CameraImage image) async {
    ...
    try {
      ...
      // Определяем, нужно ли запускать ML Kit (каждый N-й кадр)
      _mlProcessCounter++;
      final shouldProcessML = _mlProcessCounter % _mlProcessInterval == 0;
      
      if (!shouldProcessML) {
        // Быстрый путь без ML Kit - используем предыдущие данные
        final now = DateTime.now();
        if (now.difference(_lastPoseLog).inMilliseconds >= 1000) {
          print('🚀 ULTRA-FAST mode: fps=${_currentFps.toStringAsFixed(1)} (skipped ML)');
          _lastPoseLog = now;
        }
      ...
      // Создаем InputImage с кэшированием
      final inputImage = await _createInputImageUltraFast(image);
      if (inputImage == null) {
        print('❌ Failed to create InputImage (ultra-fast)');
        return;
      }
      ...
      if (now.difference(_lastPoseLog).inMilliseconds >= 1000) {
        ...
        print('🧍 ULTRA poses=${poses.length} landmarks=$landmarksCount fps=${_currentFps.toStringAsFixed(1)} ML=✅');
        _lastPoseLog = now;
      }
      ...
      if (mounted) {
        setState(() {
          ...
          if (poses.isEmpty) {
            ...
            final confidence = _calculatePoseConfidenceFast(pose);
  ...
  /// Ультра-быстрое создание InputImage с кэшированием буферов
  Future<InputImage?> _createInputImageUltraFast(CameraImage image) async {
    ...
    try {
      ...
      // Обновляем параметры для отрисовки
      if (_inputImageSize != size || _inputImageRotation != rotation) {
        _inputImageSize = size;
        _inputImageRotation = rotation;
      }

      // Очищаем кэш каждые 5 секунд для экономии памяти
      final now = DateTime.now();
      if (now.difference(_lastCacheClean).inSeconds > 5) {
      ...
      if (formatGroup == ImageFormatGroup.yuv420) {
        final bytes = await _ultraFastYuv420ToNv21(image);
      ...
      print('❌ Error creating ultra-fast InputImage: $e');
      return null;
    }
  }

  /// Ультра-быстрая конвертация YUV420 в NV21 с кэшированием буферов
  Future<Uint8List?> _ultraFastYuv420ToNv21(CameraImage image) async {
    try {
      final width = image.width;
      final height = image.height;
      final cacheKey = '${width}x${height}';
      
      // Получаем или создаем буфер из кэша
      Uint8List buffer;
      if (_imageBufferCache.containsKey(cacheKey)) {
        buffer = _imageBufferCache[cacheKey]!;
      } else {
        buffer = Uint8List(width * height + (width * height ~/ 2));
        _imageBufferCache[cacheKey] = buffer;
      }
      ...
      // Оптимизированное копирование Y плоскости
      int bufferIndex = 0;
      ...
      if (yPixelStride == 1 && yRowStride == width) {
        // Быстрое копирование блоком для непрерывных данных
        buffer.setRange(0, width * height, yBytes);
        bufferIndex = width * height;
      } else {
        // Построчное копирование с оптимизацией
        for (int y = 0; y < height; y++) {
          final rowStart = y * yRowStride;
          final rowEnd = rowStart + width * yPixelStride;
          for (int i = rowStart; i < rowEnd; i += yPixelStride) {
            buffer[bufferIndex++] = yBytes[i];
          }
        }
      }

      // Быстрое копирование UV плоскостей (NV21: VUVUVU...)
      ...
      for (int y = 0; y < height ~/ 2; y++) {
        final uRowStart = uRowStride * y;
        final vRowStart = vRowStride * y;
      ...
      print('❌ Ultra-fast YUV conversion error: $e');
      return null;
    }
  }

  /// Быстрый расчет уверенности позы
  double _calculatePoseConfidenceFast(Pose pose) {
  ...
  /// Ультра-быстрый анализ упражнений
  void _analyzeExerciseUltraFast(List<Pose> poses) {
    if (poses.isEmpty) return;
    
    final pose = poses.first;
    
    // Быстрый расчет уверенности только по ключевым точкам
    final confidence = _calculatePoseConfidenceFast(pose);
    _formScore = confidence.clamp(0, 100);
